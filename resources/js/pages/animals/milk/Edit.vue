<template>
    <Head :title="`Edit milk record – ${animal.animal_id}`" />
    <AppLayout>
        <div class="p-6 sm:p-8">
            <div class="w-full">
                ...
                <h1 class="mt-2 text-2xl font-bold text-stone-900 sm:text-3xl dark:text-stone-100">Edit milk record</h1>
                <p class="mt-1 text-stone-500 dark:text-stone-400">Update milk record for {{ animal.animal_id }}.</p>
...
const props = defineProps({
    ...
    milkRecord: { type: Object, required: true },
});

const form = useForm({
    record_date: props.milkRecord.record_date ? new Date(props.milkRecord.record_date).toISOString().slice(0, 10) : '',
    quantity_liter: props.milkRecord.quantity_liter,
    notes: props.milkRecord.notes ?? '',
});

function submit() {
    form.put(`/animals/${props.animal.id}/milk/${props.milkRecord.id}`);
}
</script>
