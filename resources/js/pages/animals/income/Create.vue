<template>
    <Head :title="`Add income – ${animal.animal_id}`" />
    <AppLayout>
        <div class="p-6 sm:p-8">
            <div class="w-full">
                ...
                <h1 class="mt-2 text-2xl font-bold text-stone-900 sm:text-3xl dark:text-stone-100">Add income</h1>
                <p class="mt-1 text-stone-500 dark:text-stone-400">Record income for {{ animal.animal_id }}.</p>
                ...
                <form class="mt-8 space-y-6 rounded-xl border border-stone-200/80 bg-white p-6 shadow-sm dark:border-stone-700 dark:bg-stone-800 sm:p-8" @submit.prevent="submit">
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            <label for="quantity_liter" class="block text-sm font-medium text-stone-700 dark:text-stone-300">Quantity (L) – for milk</label>
                            <input
                                id="quantity_liter"
...
function submit() {
    form.post(`/animals/${props.animal.id}/income`);
}
</script>
