<template>
    <Head :title="`Expense – ${animal.animal_id}`" />
    <AppLayout>
            ...
            <div class="w-full">
                ...
                <div class="mt-2 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-stone-500 dark:text-stone-400">Expense transactions for {{ animal.animal_id }}.</p>
                    </div>
                    <Link
                        :href="`/animals/${animal.id}/expense/create`"
                        ...
                        <tbody class="divide-y divide-stone-200 dark:divide-stone-700">
                            <tr v-for="item in expense.data" :key="item.id" class="bg-white dark:bg-stone-800">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 text-right text-sm">
                                    <Link
                                        :href="`/animals/${animal.id}/expense/${item.id}/edit`"
                            ...
                            <tr v-if="!expense.data || expense.data.length === 0">
                                <td colspan="4" class="px-4 py-8 text-center text-sm text-stone-500 dark:text-stone-400">No expense recorded yet.</td>
                            </tr>
                        </tbody>
                    </table>
                    <div v-if="expense.links && expense.links.length > 3" class="border-t border-stone-200 px-4 py-2 dark:border-stone-700">
                        <Pagination :links="expense.links" />
                    </div>
                </div>
            </div>
        </div>
...
function confirmDelete(item) {
    openConfirmDelete({
        ...
        message: `Delete this expense of ${formatMoney(item.amount)}?`,
        onConfirm: () => {
            router.delete(`/animals/${props.animal.id}/expense/${item.id}`);
        },
    });
}
</script>
