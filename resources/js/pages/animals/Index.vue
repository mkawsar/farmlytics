            ...
            <div class="w-full">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-2 text-2xl font-bold text-stone-900 sm:text-3xl dark:text-stone-100">Animals</h1>
                        <p class="mt-1 text-stone-500 dark:text-stone-400">Manage animals in {{ shed.name }}.</p>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <FwbButton
                            :href="`/farms/${farm.id}/sheds/${shed.id}/animals/create`"
                            color="green"
                            size="sm"
                            class="w-fit"
                        >
                ...
                <DataTable class="mt-6">
                    <Table>
                        <template #head>
                            ...
                            <FwbTableHeadCell>Group</FwbTableHeadCell>
                            <FwbTableHeadCell>
                        ...
                        <FwbTableRow v-for="animal in animals.data" :key="animal.id">
                            ...
                            <FwbTableCell>{{ groupLabel(animal.grouping) }}</FwbTableCell>
                        ...
                        <FwbTableRow v-if="!animals.data || animals.data.length === 0">
                            ...
                            >
                                <div class="flex min-h-[280px] flex-col items-center justify-center gap-1">
                                    ...
                                    <Link :href="`/farms/${farm.id}/sheds/${shed.id}/animals/create`" class="font-medium text-green-600 hover:underline dark:text-green-500">Add your first animal</Link>
                                </div>
                            </FwbTableCell>
                        </FwbTableRow>
                    </Table>
...
function submitSearch() {
    router.get(`/farms/${props.farm.id}/sheds/${props.shed.id}/animals`, { search: searchQuery.value || undefined }, { preserveState: false });
}
...
function confirmBulkDelete() {
    ...
    openConfirmDelete({
        ...
        onConfirm: () => {
            router.post(`/farms/${props.farm.id}/sheds/${props.shed.id}/animals/bulk-destroy`, { ids: selectedIds.value }, { preserveScroll: true });
            selectedIds.value = [];
        },
    });
}
