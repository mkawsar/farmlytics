<template>
    <Head :title="`Edit ${animal.animal_id}`" />
    <AppLayout>
        <div class="p-6 sm:p-8">
            <div class="w-full">
                ...
                <h1 class="mt-2 text-2xl font-bold text-stone-900 sm:text-3xl dark:text-stone-100">Edit animal</h1>
                <p class="mt-1 text-stone-500 dark:text-stone-400">Update {{ animal.animal_id }}.</p>
...
const form = useForm({
    animal_id: props.animal.animal_id ?? '',
    breed: props.animal.breed ?? '',
    gender: props.animal.gender ?? 'male',
    status: props.animal.status ?? 'active',
    grouping: props.animal.grouping ?? '',
    date_of_birth: formatDate(props.animal.date_of_birth),
    purchase_date: formatDate(props.animal.purchase_date),
    purchase_price: props.animal.purchase_price ?? '',
    current_weight: props.animal.current_weight ?? '',
});
